...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'Anuudek'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
}, 30 * 1000);
}}
...
try {
let listaDirectorios = readdirSync('./Anuudek/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./Anuudek/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./Anuudek/${directorio}`).filter(fileInDir => {
