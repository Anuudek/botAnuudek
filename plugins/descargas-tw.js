...
await conn.sendMessage(m.chat, {text: waitttt, edit: key});
     const captionFG = `✨ qualidade » ${quality}\n${wm}`
 ...
 try {
   ...
   for (let x = 0; x < res.media.length; x++) {
     const caption = x === 0 ? res.caption.replace(/https:\/\/t.co\/[a-zA-Z0-9]+/gi, '').trim() : `✨ qualidade» ${quality}\n${wm}`
     await conn.sendFile(m.chat, res.media[x].url, 'error.mp4', caption, m);
   }
 } catch {
   try {
     const AA = await savefrom(text);
     await conn.sendFile(m.chat, AA.url[0].url, 'error.mp4', `*✨ qualidade » ${quality}\n${wm}`, m);
   } catch {
     conn.sendMessage(m.chat, {text: `${lenguajeGB['smsAvisoFG']()} ocorreu um erro, tente novamente.`, edit: key});
 handler.limit = false
   }
  }
 }
